...
def skip(
    ...
    for i in range(len(num_channels_down)):
        ...
        if i > 1:
            deeper.add(NONLocalBlock2D(in_channels=num_channels_down[i]))  # 一种注意力机制模块
        ...
        if i == len(num_channels_down) - 1:
            ...
            deeper.add(deeper_main)  # deeper_main 放入到 deeper
            k = num_channels_up[i + 1]

        deeper.add(nn.Upsample(scale_factor=2, mode=upsample_mode[i]))  # 2*2 变成 4 * 4
