...
from networks.myskip import My_Skip
from networks.skip import skip
from networks.fcn import fcn
from networks.fcn import FcnNet
import cv2
import torch
import torch.optim
...
parser.add_argument('--data_path', type=str, default="datasets/mytest", help='path to blurry image')
...
for f in files_source:
    ...
    if imgname.find('liugang') != -1:
        opt.kernel_size = [35, 35]
    # else:
    #     opt.kernel_size = [35, 35]
    ...
    net_input = get_noise(input_depth, INPUT, (opt.img_size[0], opt.img_size[1])).type(
        dtype)  # torch.Size([1, 8, 598, 898])
    ...
    n_k = 200
    net_input_kernel = get_noise(n_k, INPUT, (1, 1)).type(dtype)  # (1,200,1,1)
    net_input_kernel.squeeze_()

    net_kernel = FcnNet(n_k, opt.kernel_size[0] * opt.kernel_size[1]).cuda()
    ...
    for step in tqdm(range(num_iter)):
        ...
        out_x = net(net_input)  # net_input.shape = (1,8,598,898)
        out_k = net_kernel(net_input_kernel)    # 生成模糊核线性参数

        out_k_m = out_k.view(-1, 1, opt.kernel_size[0], opt.kernel_size[1])

        # print(out_k_m)  将out_k_m作为卷积核
